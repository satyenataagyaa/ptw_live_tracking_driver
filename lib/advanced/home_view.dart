...
class HomeViewState extends State<HomeView>
  ...
  // Publish location
  Future<void> publishLocation(bg.Location location) async {
    const publish = 'publish';
    const graphQLDocument = '''
      mutation publish(\$latitude: Float!, \$longitude: Float!, \$tripId: String!) {
        $publish(latitude: \$latitude, longitude: \$longitude, tripId: \$tripId) {
          tripId
          latitude
          longitude
        }
     ...
     ''';
    final publishRequest = GraphQLRequest<Location>(
    ...
    final response = await Amplify.API.mutate(request: publishRequest).response;
    safePrint('Publish result: $response');
  }
  ...
  void _onProviderChange(bg.ProviderChangeEvent event) async {
    ...
    // var authorizationStatus = event.status;
    // var map = {
    //   "authorizationStatus": AuthorizationStatus.values[authorizationStatus],
    // };
  ...
  void _onClickTestMode() {
    ...
    _testModeTimer = new Timer(const Duration(seconds: 2), () {
      _testModeClicks = 0;
    });
  }
